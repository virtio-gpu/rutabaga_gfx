...
use crate::protocol::MagmaCreateQueueInfo;
use crate::protocol::MagmaSubmitInfo;
...
impl AddressSpace for WddmAddressSpace {}
...
impl WddmAdapter {
    ...
    pub fn initialize(&mut self) -> MagmaGpuResult<MagmaPhysicalDeviceInfo> {
        ...
        if self.segment_group_size.NonLocalMemory > 0 {
            mem_props.add_heap(self.segment_group_size.NonLocalMemory, 0);
        ...
        if self.segment_group_size.LocalMemory > 0 {
            mem_props.add_heap(
                self.segment_group_size.LocalMemory,
        ...
        info.queue_family_count = 1;
        info.queue_families[0] = MagmaQueueFamilyProperties::new(
...
impl GenericDevice for WddmDevice {
    ...
    fn create_address_space(self: Arc<Self>) -> MagmaGpuResult<Arc<dyn AddressSpace>> {
        Ok(Arc::new(WddmAddressSpace { _device: self }))
    }
    ...
    fn create_buffer(
        ...
        let buf = WddmBuffer::new(self.clone(), create_info, &self.mem_props)?;
        Ok(Arc::new(buf))
    }
    ...
    fn import(self: Arc<Self>, info: MagmaImportHandleInfo) -> MagmaGpuResult<Arc<dyn Buffer>> {
        ...
        let buf = WddmBuffer::from_existing(self, open_alloc_info.hAllocation, info.size)?;
        Ok(Arc::new(buf))
    }
}
...
impl WddmQueue {
    pub fn new(device: Arc<dyn Device>) -> MagmaGpuResult<WddmQueue> {
...
impl SyncObject for WddmSyncObject {}
...
unsafe impl Send for WddmQueue {}
unsafe impl Sync for WddmQueue {}
